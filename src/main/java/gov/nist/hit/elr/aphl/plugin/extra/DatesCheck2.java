...
public class DatesCheck2 {
	...
	private static final Logger logger = LogManager.getLogger(DatesCheck2.class);

	private String defaultTz = "";

	/**
	 * 
	 * If a group does not have an SPM segment, OBR/OBX date must be checked against
	 * the other groups SPMs
	 * 
	 * (OBR.7 = OBX.14 = SPM.17.1) <= (SPM.17.2 = OBR.8) <= SPM.18 <= OBX.19 <=
	 * OBR.22
	 */
	...
	public java.util.List<String> assertionWithCustomMessages(Element e) {
		...
		// we extract the SPM values from all oo groups in the message. If no SPM, we
		// don't check
		// anything
	...
	private java.util.List<Map<String, LocationDate>> extractSPMvalues(Element m) {
		java.util.List<Map<String, LocationDate>> spmValues = new java.util.ArrayList<Map<String, LocationDate>>();
				...
				if ((a.getLocation().equals("OBR-7") || a.getLocation().equals("OBX-14")
					...
					if (!ext1.isEqual(ext2)) {
						result.add(String.format(
								...
								a.getLocation(), a.getLine(), a.getDate(), b.getLocation(), b.getLine(), b.getDate(),
