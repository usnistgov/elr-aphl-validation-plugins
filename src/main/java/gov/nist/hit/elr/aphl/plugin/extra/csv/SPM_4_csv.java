...
public class SPM_4_csv implements SPM_4 {
	...
	public String getSPM4_CSV() {
		return ELR_FOUNDATION.getSPM4_CSV();
	}
	...
	public java.util.List<String> check(ComplexCodedElement SPM4) throws IOException {
		...
		util.parseSpecimenTypeCSV(getSPM4_CSV());
		...
		if (!util.getSPM4().contains(identifierSPM4) && !util.getSPM4().contains(alternateSPM4)) {
			String st = "SPM-4 (Specimen Type) does not contain a valid code for the " + getProgram() + " program.";
			messages.add(st);
