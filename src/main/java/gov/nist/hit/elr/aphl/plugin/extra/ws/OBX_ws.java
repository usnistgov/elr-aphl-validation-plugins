...
public abstract class OBX_ws implements OBX {
	...
	public java.util.List<String> checkOBX3(ComplexCodedElement obx3)
		...
		int indentifierCt = Test.findByOBX3(identifier.getIdentifier(), identifier.getCodeSystem(), tests).size();
		int alternateCt = Test.findByOBX3(alternate.getIdentifier(), alternate.getCodeSystem(), tests).size();

		if (indentifierCt == 0 && alternateCt == 0) {
	...
	public java.util.List<String> checkOBX3_OBX2(ComplexCodedElement obx3, String obx2)
		...
		Set<Test> expectedTests = Test.findByOBX3(identifier.getIdentifier(), identifier.getCodeSystem(), tests);
		if (expectedTests.size() > 0) {
			Set<String> expectedValues = expectedTests.stream().map(expectedTest -> expectedTest.getObx2())
			...
			if (!expectedValues.contains(obx2)) {
				messages.add(
						"The OBX-2 value (" + obx2 + ") is not valid for this OBX-3 value (" + identifier.prettyPrint()
								+ "). The expected OBX-2 value is one of " + expectedValues.toString() + "");
			}
		}
	...
	public java.util.List<String> checkOBX3_OBX5_CWE(ComplexCodedElement obx3, ComplexCodedElement obx5)
		...
		java.util.List<Test> tests = ws.getTests(getWEBSERVICE_URL());
		ValueSets valueSets = ws.getValueSets(getVOCAB_WEBSERVICE_URL());

		// get the expected Tests for this OBX-3 value
		Set<Test> expectedTests = Test.findByOBX3(identifier.getIdentifier(), identifier.getCodeSystem(), tests);

		// TODO : do we want to add an extra layer to check for OBX-2 value ? CAn the
		// value set be
		...
		if (expectedTests.size() > 0) {
			...
			if (normalizedValueSets.size() == 0) {
				messages.add("Invalid value set identifier(s) " + expectedValueSets.toString() + " for OBX-3 value "
						+ identifier.prettyPrint() + ". The tool cannot validate the OBX-5 value.");
			}

			// check that one of the value sets contains the OBX-5 identifier (or the OBX-5
			// alternate)
			for (String normalizedValueSet : normalizedValueSets) {
				java.util.List<ExpandedValueSet> valueSetList = ws.getExpandedValueSet(getVOCAB_WEBSERVICE_URL(),
						normalizedValueSet);
