...
public abstract class OBX3_OBR4_Warning_ws implements OBX3_OBR4_Warning {
	...
	public abstract String getWEBSERVICE_URL();

	public abstract String getVOCAB_WEBSERVICE_URL();
	...
	public java.util.List<String> check(ComplexCodedElement OBR4, java.util.List<ComplexCodedElement> OBX3s)
		...
		java.util.List<Test> tests = ws.getTests(getWEBSERVICE_URL());
		java.util.List<Order> orders = ws.getOrders(getWEBSERVICE_URL());
		java.util.List<Observation> observations = ws.getObservations(getWEBSERVICE_URL());
		...
		for (ComplexCodedElement OBX3 : OBX3s) {
			...
			long identifierOBX3Count = Test.countByOBX3(identifierOBX3.getIdentifier(), identifierOBX3.getCodeSystem(),
					tests);
			long alternateOBX3Count = Test.countByOBX3(alternateOBX3.getIdentifier(), alternateOBX3.getCodeSystem(),
					tests);
			...
			if (obx3InTests) {
				...
				Set<Test> subsetId = Test.findByOBR4(identifierOBR4.getIdentifier(), identifierOBR4.getCodeSystem(),
						obr4Expected);
				Set<Test> subsetAlt = Test.findByOBR4(alternateOBR4.getIdentifier(), alternateOBR4.getCodeSystem(),
						obr4Expected);
				...
				long identifierOBR4Count = Test.countByOBR4(identifierOBR4.getIdentifier(),
						identifierOBR4.getCodeSystem(), tests);
				...
				long alternateOBR4Count = Test.countByOBR4(alternateOBR4.getIdentifier(), alternateOBR4.getCodeSystem(),
						tests);
				...
				identifierOBR4Count = Order.count(identifierOBR4.getIdentifier(), identifierOBR4.getCodeSystem(),
						orders);
				if (identifierOBR4Count > 0) {
				...
				alternateOBR4Count = Order.count(alternateOBR4.getIdentifier(), alternateOBR4.getCodeSystem(), orders);
				if (alternateOBR4Count > 0) {
					// TODO should be a warning
					messages.add("[WARNING] The OBR-4 value (" + alternateOBR4.prettyPrint()
							+ ") should not be associated with this OBX-3 value (" + obx3.prettyPrint() + ")");
			...
			if (!obx3InTests) {
				...
				identifierOBX3Count = Observation.count(identifierOBX3.getIdentifier(), identifierOBX3.getCodeSystem(),
						observations);
				alternateOBX3Count = Observation.count(alternateOBX3.getIdentifier(), alternateOBX3.getCodeSystem(),
						observations);
