...
public class ValueSetsGenerator {
	...
	public static void main(String[] args) throws IOException {
		...
		// value sets
		util.parseValueSetsCSV(ARLN.getVALUE_SETS_CSV());
		Map<String, Set<CodedElement>> map = util.getValueSets();
		List<String> vsNames = Arrays.asList("Acinetobacter culture result", "ARLN microorganism", "ARLN Regional Labs",
				"Beta-lac results for GC", "C auris culture result", "C auris PCR result",
				"Carbapenemase phenotypic results", "Clinical sample or isolate", "CRE AST method", "CRE PCR method",
				"CRE PCR results", "CRE Phenotypic method", "GC Facility codes", "GC specimen quality", "Gender",
				"Healthcare facility type", "Identifier type", "Index case resistance gene", "Index case species",
				"Jurisdiction PHL codes for GC", "PHL - CRE CRPA", "Reason for screening", "Yes No Indicator",
				"Yes No Unknown");
		...
		try {
			...
			for (String vsName : vsNames) {
				...
				myWriter.write("<ValueSetDefinition BindingIdentifier=\"" + formattedName + "\" Name=\"" + vsName
						+ "\" Description=\"" + vsName
		...
		// constraints
		util.parseTestCSV(ARLN.getTEST_CSV());
		...
		try {
			...
			for (CodedElement obx3 : obx2Map.keySet()) {
				...
				if (obx2.size() == 1) {
					myWriter.write("<Description>If the value of OBX-3[1] (Observation Identifier) is '"
							+ obx3.getIdentifier() + "', then the value of OBX-2[1] (Value Type) SHALL be '"
							+ StringUtils.join(obx2, "") + "'.</Description>\n");
				} else {
					myWriter.write("<Description>If the value of OBX-3[1] (Observation Identifier) is '"
							+ obx3.getIdentifier() + "', then the value of OBX-2[1] (Value Type) SHALL be one of ["
							+ StringUtils.join(obx2, ",") + "].</Description>\n");
				}
				...
				myWriter.write("<PlainText Path=\"3[1].1[1]\" Text=\"" + obx3.getIdentifier()
						+ "\" IgnoreCase=\"false\" />\n");
				myWriter.write("<Presence Path=\"3[1].3[1]\" />\n");
				myWriter.write("<PlainText Path=\"3[1].3[1]\" Text=\"" + obx3.getCodeSystem()
						+ "\" IgnoreCase=\"false\" />\n");
				myWriter.write("</FORALL>\n");
				myWriter.write("<AND>\n");
				myWriter.write("<Presence Path=\"2[1]\" />\n");
				myWriter.write("<StringList Path=\"2[1]\" CSV=\"" + StringUtils.join(obx2, ",")
						+ "\" IgnoreCase=\"false\" />\n");
				myWriter.write("</AND>\n");
				myWriter.write("</IMPLY>\n");
				myWriter.write("</Assertion>\n");
				...
				if (obx5Map.containsKey(obx3) && !"".equals(obx5Map.get(obx3))) {
					...
					myWriter.write("<Description>If the value of OBX-3[1] (Observation Identifier) is '"
							+ obx3.getIdentifier() + "', then the value of OBX-5[1] () SHALL come from the '"
							+ obx5Map.get(obx3) + "' value set.</Description>\n");
					myWriter.write("<Assertion>\n");
					myWriter.write("<IMPLY>\n");
					myWriter.write("<FORALL>\n");
					myWriter.write("<Presence Path=\"3[1].1[1]\" />\n");
					...
					myWriter.write("<ValueSet ValueSetID=\"" + obx5Map.get(obx3)
							+ "\" Path=\"5[1]\" BindingLocation=\"1\" BindingStrength=\"R\"/>\n");
					myWriter.write("</AND>\n");
					myWriter.write("</IMPLY>\n");
					myWriter.write("</Assertion>\n");
					myWriter.write("</Constraint>\n");
